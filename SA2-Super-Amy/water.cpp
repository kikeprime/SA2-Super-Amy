...
void __cdecl SS_Water_Main(ObjectMaster* obj)
{
	...
	if (((co2->Upgrades & Upgrades_SuperSonic) == 0) || co2->Powerups & Powerups_Dead || co2->UnderwaterTime > 0 || player->Action == Action_Swim || player->Action == Action_SwimMove || player->Action == Action_SwimSink || player->Action == Action_BounceDown && !isFakeWaterLevel())
	{
		if(co2->CharID2 != Characters_Tails)
			waterColTask = 0;
...
void LoadWaterTextures(char charID) {
	...
	if (charID == Characters_Sonic || charID == Characters_Amy) {
