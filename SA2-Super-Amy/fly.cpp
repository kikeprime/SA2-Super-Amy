...
void SS_EnableFly_CheckInput(EntityData1* data1, CharObj2Base* co2, char pID) {
	...
	if (Controllers[pID].press & FlightButton)
	{
		...
		bool isAmy = co2->CharID2 == Characters_Amy;
		bool isMetal = co2->CharID2 == Characters_MetalSonic;
		...
		PlayVoice(3, rng);
		if(isAmy)
		SS_SetFlyNextAction(data1, co2, (char)SSFly::Ascending, (__int16)saBeginDash2);
		else
...
void SS_Standing(EntityData1* data1, CharObj2Base* co2)
{
	...
	if ((0.0 != AnalogThings[co2->PlayerNum].magnitude) || (data1->Status & Status_DisableControl))
	{
		bool isAmy = co2->CharID2 == Characters_Amy;
		if(isAmy)
		SS_SetFlyNextAction(data1, co2, (char)SSFly::Moving, (__int16)superAmyFlying1);
		else
	...
	if (Jump_Held[pnum])
	{
		...
		bool isAmy = co2->CharID2 == Characters_Amy;
		if(isAmy)
		SS_SetFlyNextAction(data1, co2, (char)SSFly::AscendingIntro, (__int16)saBeginAscent);
		else
	...
	bool isAmy = co2->CharID2 == Characters_Amy;
	if(isAmy)
	SS_SetFlyNextAction(data1, co2, (char)SSFly::DescendingIntro, (__int16)saBeginDescent);
	else
...
void SS_Moving(EntityData1* data1, CharObj2Base* co2)
{
	...
	if (Jump_Held[pnum])
	{
		...
		bool isAmy = co2->CharID2 == Characters_Amy;
		if(isAmy)
		SS_SetFlyNextAction(data1, co2, (char)SSFly::Ascending, (__int16)saBeginDash);
		else
